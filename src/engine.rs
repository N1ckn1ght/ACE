...
use crate::{util::*, board::Board, chara::Chara};

/* TODO | IN DEV | ETC */
/* THIS IS STRICTLY TEMPORARY SEARCH SOLUTION SIMPLY IN ORDER TO TEST EVAL FUNCTION
    DO
    NOT
    PUT
    AS
    RELEASE
*/
// Planned: ITERATIVE DEEPENING BASED ON A TIME SPENT!
...
fn correct_mate(eval: &mut Eval, board: &Board) -> bool {
    if eval.mate == 0 {
        return false;
    }
    let mut dist = eval.depth - board.no;
    if eval.mate < 0 {
        dist = -dist;
    }
    eval.mate += dist;
